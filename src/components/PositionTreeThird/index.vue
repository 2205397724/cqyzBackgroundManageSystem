        ...
        <div class="houses p-l-20">
            <div style="width: 100%;">
                <div v-for="item in data_tab.arr" :key="item.id" class="housesStyle">
                    ...
                    <span v-for="items in item.houses" :key="items.id" class="floorHouse">
                        <span :class="[isSelect ? 'hover' : '']" @click="addHouses(items.id)">{{ items.name }}</span>
                    </span>
                </div>
            </div>
        </div>
...
const loadNode = (node, resolve) => {
    ...
    //             if (res.status == 200) {
    //                 for (let i in res) {
    //                     tree_arr.push({ name: res[i].name, type: 'region', next_type: 'region', id: res[i].code,isLeaf:true})
    //                 }
    //             }
    //             resolve(tree_arr)
    //             emit('checkFunc', { 0: tree_item.value, 1: treeDetail.arr })
