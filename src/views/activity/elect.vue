                    ...
                    <el-table-column fixed="right" width="250px" label="操作">
                        <template #default="scope">
                            ...
                            <el-link :underline="false" type="primary">
                                <router-link class="el-button details" :to="{name: 'electDetail',query:{ id : scope.row.id }}">详情</router-link>
                            </el-link>
                            <el-popconfirm
...
let total = ref(100)
let per_page = ref(15)
let page = ref(Number(sessionStorage.getItem('elect_currentPage')) || 1)
// 详情
...
// Tabs标签页点击切换事件,切换显示不同状态的选举
// 切换标签后，根据label的值进行if判断，切换不同状态选举
const index = ref(0)
...
// 监听分页
watch(page, () => {
    sessionStorage.setItem('elect_currentPage', page.value)
    getTabListFunc()
})
onBeforeRouteLeave((to, from) => {

    if (to.meta.title == '选举详情') {
        return true
    } else {
        sessionStorage.removeItem('elect_currentPage')
    }
})
const surveyId = ref('')
// 同意拒绝提交
