...
let total = ref(100)
let per_page = ref(15)
let page = ref(Number(sessionStorage.getItem('jointly_currentPage')) || 1)
// 详情
...
// 监听分页
watch(page, () => {
    sessionStorage.setItem('jointly_currentPage', page.value)
    getTabListFunc()
})
import { onBeforeRouteLeave } from 'vue-router'
onBeforeRouteLeave((to, from) => {

    if (to.meta.title == '联名详情') {
        return true
    } else {
        sessionStorage.removeItem('jointly_currentPage')
    }
})
const surveyId = ref('')
// 同意拒绝提交
