        ...
        <page-main>
            <div>
                ...
                <p class="size-sm sno">
                    ...
                    <el-tag v-show="dataForm.item.status == 1" class="m-t-10 m-l-10 btnNone" type="primary" size="small">{{ getOptVal(opts_all.obj.announce_status,dataForm.item.status) }} </el-tag>
                    <el-tag v-show="dataForm.item.status == 2" class="m-t-10 m-l-10 btnNone noDeal" type="warning" size="small">{{ getOptVal(opts_all.obj.announce_status,dataForm.item.status) }} </el-tag>
                    <el-tag v-show="dataForm.item.status == 3" class="m-t-10 m-l-10 btnNone" type="warning" size="small">{{ getOptVal(opts_all.obj.announce_status,dataForm.item.status) }} </el-tag>
                    <el-tag v-show="dataForm.item.status == 4" class="m-t-10 m-l-10 btnNone" type="success" size="small">{{ getOptVal(opts_all.obj.announce_status,dataForm.item.status) }} </el-tag>
                    <el-tag v-show="dataForm.item.status == 5" class="m-t-10 m-l-10 btnNone" type="info" size="small">{{ getOptVal(opts_all.obj.announce_status,dataForm.item.status) }} </el-tag>
                    <el-tag v-show="dataForm.item.status == 6" class="m-t-10 m-l-10 btnNone" type="info" size="small">{{ getOptVal(opts_all.obj.announce_status,dataForm.item.status) }} </el-tag>
                    <el-tag v-show="dataForm.item.status == 7" class="m-t-10 m-l-10 btnNone" type="info" size="small">{{ getOptVal(opts_all.obj.announce_status,dataForm.item.status) }} </el-tag>
                </p>
            </div>
            <div>
                <el-tabs v-model="activeName" @tab-click="handleClick">
                            ...
                            <el-row :gutter="20">
                                <el-col :span="8">
                                    <div v-if="dataForm.item.affix&&dataForm.item.affix.length>0" class="item">
                                        ...
                                        <div class="right">
                                            <div v-for="(item,i) in dataForm.item.affix" :key="i" class="inline-block m-r-10">
                                                <el-tag type="success">
                                                    <el-link type="success" class="link" :href="dataForm.item.affixs" target="_blank">{{ item.title }}</el-link>
                                                </el-tag>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <el-row :gutter="20">
                                ...
                                <el-col :span="8">
                                    <div class="item">
                                        ...
                                        <div class="right">{{ getOptVal(opts_all.obj.article_lv,dataForm.item.totype) }}</div>
                                    </div>
                                </el-col>
                                <el-col :span="8">
                            ...
                            <el-row :gutter="20">
                                ...
                                <el-col :span="8">
                                    <div class="item">
                                        ...
                                        <div class="right">
                                            <span>{{ dataForm.item.uinfo?.name?dataForm.item.uinfo?.name:dataForm.item.uinfo?.nickname? dataForm.item.uinfo?.nickname:dataForm.item.uinfo?.username }}</span>
                    ...
                    <el-tab-pane label="审核记录" name="2">
                        <el-scrollbar :height="article_tab.arr.length >= 2 ? '500px': '300px'">
                            <div>
                                <el-timeline>
                                    <el-timeline-item v-for="(item,index) in article_tab.arr" :key="index" :timestamp="item.created_at" placement="top" :type="index==0? 'primary':''">
                                        <el-card>
                                            <div class="details-box">
                                                <div class="item">
